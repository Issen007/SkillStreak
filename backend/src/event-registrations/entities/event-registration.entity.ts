...
export class EventRegistration {
  ...
  /**
   * When we asked this person whether they want release news, or null.
   *
   * Only the one-off re-consent campaign writes it — see
   * `askForReleaseConsent`. Existing registrants agreed to a demo
   * invitation and nothing more, so they have to be asked before they can
   * be added, and asking twice is the failure this column prevents.
   */
  @Column({
    name: 'release_consent_asked_at',
    type: 'timestamptz',
    nullable: true,
  })
  releaseConsentAskedAt!: Date | null;
